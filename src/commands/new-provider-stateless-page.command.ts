...
import {
  ...
  widgetTemplate,
  stateTemplate,
} from "../templates/provider-stateless.template";

export const newProviderStatelessPage = async (uri: Uri) => {
  console.log(uri);
  ...
  if (pageName.length > 50) {
    window.showErrorMessage("The name is too long. Please enter less than 50 characters or less");
    return;
  }
...
async function generateCode(pageName: string, targetDirectory: string) {
  ...
  if (!existsSync(pageDirectoryPath)) {
    ...
    await Promise.all([
      ...
      statelessTemplate(pageName, targetDirectory),
